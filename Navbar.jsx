...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { id: 'timeline', label: lang === 'ne' ? 'टाइमलाइन' : lang === 'ko' ? '타임라인' : 'Timeline' },
    { id: 'family', label: t.nav.family },
    { id: 'gallery', label: t.nav.gallery },
    { id: 'village', label: t.nav.village },
    { id: 'events', label: lang === 'ne' ? 'कार्यक्रम' : lang === 'ko' ? '행사' : 'Events' },
    { id: 'recipes', label: lang === 'ne' ? 'रेसिपी' : lang === 'ko' ? '레시피' : 'Recipes' },
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        {/* Logo */}
        <button onClick={() => scrollTo('home')} className="font-serif text-xl font-semibold text-foreground tracking-wide">
          सागर<span className="text-primary">.</span>
        </button>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link
            ...
            className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"
          >
            {lang === 'ne' ? 'जीवनी' : lang === 'ko' ? '전기' : 'Biography'}
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={toggleDark}
            className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-all"
